...
export default function SideBar({sidebarController}) {
  ...
  return (
    <aside id="slidebar"  onClick={() => setSidebarOpen(true)}  className={`bg-gray-sl text-gray-100 border-gray-100 border-e ${
      ...
      <ul className={`flex flex-col p-4 gap-2 `}>
        ...
        <div>
        <li onClick={() => handleOpen(1)} className={`hover:bg-white relative ${open == 1 ? "ps-2 bg-white" : ""}`}>
          <a className="flex gap-2 items-center whitespace-nowrap py-2 bg-gray-sl dark:bg-gray-700 transform duration-300 cursor-pointer" >
            ...
            <img className={`absolute right-0 top-3 ${open == 1 ? "rotate-180" : ""} `} src="../svg/flecha.svg" alt="" srcSet="" />
            </a>
        </li>

        <ul className={` bg-gray-500 overflow-hidden submenu ${open == 1 ? "h-40" : "h-0"} `}>
